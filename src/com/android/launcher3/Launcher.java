...
public class Launcher extends Activity
    ...
    private void updateColor(boolean enabled) {
        final TypedArray ta = obtainStyledAttributes(new int[]{android.R.attr.colorPrimary});
        final int primaryColor = ta.getColor(0, 0);
        ta.recycle();
        if (mAllAppsController != null) {
            mAllAppsController.setColor(primaryColor);
        }
        if (mAppsView != null) {
            mAppsView.setBackgroundColor(primaryColor);
        }
        if (mHotseat != null) {
            mHotseat.setBackgroundColor(primaryColor);
        }
