...
public class Launcher extends Activity
    ...
    private LauncherActivityInfoCompat mLauncherActivityInfoCompat;
                    ...
                    public void run() {
                        if (mWorkspace != null) {
                            ...
                            if (mEditView.getVisibility() == View.VISIBLE) {
                                RevealEditView.close(Launcher.this, false, false, null);
                            }
                        }
                    }
                });
    ...
    public void onBackPressed() {
        ...
        //close edit icon view
        if (mEditView.getVisibility() == View.VISIBLE) {
            RevealEditView.close(this, false, false, null);
        }
    ...
    protected void startEdit(final ItemInfo info) {
        ...
        mLauncherActivityInfoCompat = LauncherAppsCompat.getInstance(this)
                .resolveActivity(info.getIntent(), info.user);

        mItemInfo = info;
        mAppliedIcon = mIconsHandler.getAppliedIconBitmap(this, mIconCache, mLauncherActivityInfoCompat, info);
        ...
        boolean noIconPacks = iconPacks.second.isEmpty();

        //check if icon packs list is empty. If Yes add a view to let the user 
        //to open the PlayStore to search for icon packs
        if (noIconPacks) {
            iconPacks.second.add(0, getString(R.string.icon_pack_search));
        }

        //set the recycler view adapter and pass arguments to the adapter to it
        iconPacksRecyclerView.setAdapter(new IconPacksRecyclerViewAdapter(this, iconPacks, noIconPacks));

        mPackageIcon.setImageBitmap(mAppliedIcon);
        updateEditLabel(mIconCache.getCacheEntry(mLauncherActivityInfoCompat).title);
        ...
        RevealEditView.show(this);
    }

    void openIconChooserForIconPack(String iconPack) {
        RevealEditView.close(this, false, true, iconPack);
    }
